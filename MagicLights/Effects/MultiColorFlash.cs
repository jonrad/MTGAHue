...
namespace MagicLights.Effects
{
        ...
        : IEffect<CastSpell>
    {
        ...
        public ITransition? OnMagicEvent(CastSpell magicEvent)
        {
            ...
            {
                new FadeInTransition(new MappedLightSource(coordinateMap, .4f), new MappedLightSource(coordinateMap, .8f), 500),
                new FadeInTransition(new MappedLightSource(coordinateMap, .8f), new MappedLightSource(coordinateMap, .4f), 500)
            };

            var flashCount = 3;

            var transitions =
                new[] { new FadeInTransition(new MappedLightSource(coordinateMap, .4f), 50) }
                .Concat(Enumerable.Repeat(flash, flashCount).SelectMany(s => s))
                .Concat(new[] { new FadeInTransition(new MappedLightSource(coordinateMap, .4f), new MappedLightSource(coordinateMap, .1f), 1000) })
                .ToArray();
